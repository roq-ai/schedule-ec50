...
import { getColabs, deleteColabById } from 'apiSdk/colabs';
import { ColabInterface } from 'interfaces/colab';

type ColumnType = ColumnDef<ColabInterface, unknown>;

interface ColabListPageProps {
...
export function ColabListPage(props: ColabListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<ColabInterface>>(
    () => `/colabs?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: ColabInterface) => {
    if (hasAccess('colab', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/colabs/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/colabs/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('colab', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/colabs/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('colab', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
